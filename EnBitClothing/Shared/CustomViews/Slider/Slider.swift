...
struct RangedSliderView: View {
    ...
    @ViewBuilder private func sliderView(sliderSize: CGSize) -> some View {
        ...
        ZStack {
            RoundedRectangle(cornerRadius: 5)
                .fill(Color.custom(._FFFFFF).opacity(0.5))
                .frame(height: 5)
            ZStack {
    ...
    @ViewBuilder func lineBetweenThumbs(from: CGPoint, to: CGPoint) -> some View {
        Path { path in
            path.move(to: from)
            path.addLine(to: to)
        }.stroke(Color.custom(._6347F3), lineWidth: 5)
    }
        ...
        ZStack {
            Text(String(format: "%.0f",value))
                ...
                .background(
                    ZStack{
                        RoundedRectangle(cornerRadius: 5)
                            .fill(Color.custom(._6347F3).opacity(0.5))
                            .frame(height: 28)
                        TriangleShape()
                            .foregroundColor(Color.custom(._6347F3).opacity(0.5))
                            .frame(width: 15)
                            .offset(y: 25)
